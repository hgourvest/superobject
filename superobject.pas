...
function TSuperRttiContext.FromJson(TypeInfo: PTypeInfo; const obj: ISuperObject;
  ...
  begin
  {$IFNDEF RELEASE}
     raise Exception.Create(Context + ' Marshalling Error');
  {$ELSE}
     Result := False;
  {$ENDIF}
  end;
  ...
  begin
    case ObjectGetType(obj) of
    stObject, stArray:
      Error('String (from object or array)');
    stnull:
      begin
        Value := '';
        Result := True;
