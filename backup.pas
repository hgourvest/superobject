...
begin
  for o in r.GetAttributes do
    if o is SOName then
    begin
      outputdebugstring(pchar(SOName(o).Name));
      Exit(SOName(o).Name);
    end;
  Result := r.Name;
  ...
  begin
    case ObjectGetType(obj) of
      ...
      begin
        ...
        name:=value.AsObject.ClassName;
        delete(name, 1, pos('<', name));
        delete(name, length(name), 1);
        outputdebugstring(pchar(name));

        rType:=context.FindType(name);

        method:=Context.GetType(typeInfo).GetMethod('Add');
        Setlength(param, 1);
        for iElement:= 0 to obj.AsArray.Length- 1 do
        begin
          v:=getTypeData(rType.Handle).ClassType.Create;
          FromJson(rType.Handle, obj.AsArray.GetO(iElement), v);
          param[0]:=v;
          method.Invoke(value.AsObject, param);
        end;
